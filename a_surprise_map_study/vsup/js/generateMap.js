...
function removeRow(id){
	...
	if (counties.length) {
		counties.forEach(function(county){	
			...
			row += '<div class="row-county" id="' + county +'"><button class="btn dark-green btn-sm" id="' + id + '" type="button" onclick="removeRow(this.id)" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Click to Remove County" class="form-control btn-danger" style="font-size: 14px; vertical-align:middle;"><i class="fa fa-times"></i> '+ ct.recip_county + ', ' + ct.recip_state + '</button></div>'
		})
		...
		timeout = setTimeout(function() {
			...
			if (expType == 1 && countyData.series_complete_pop_pct != 0){
				...
				if ((count < 5) && (counties.indexOf(el.id) == -1)){
					row += '<div class="row-county" id="' + el.id +'"><button class="btn dark-green btn-sm" id="' + el.id + '" type="button" onclick="removeRow(this.id)" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Click to Remove County" class="form-control btn-danger" style="font-size: 14px; vertical-align:middle;"><i class="fa fa-times"></i> '+ county + ', ' + countyData.recip_state + '</button></div>'
					document.getElementById("rowCounties").innerHTML = row;
					count += 1
					counties.push(+el.id)	
					document.getElementById("ccount").innerText = "Selected Counties [" + counties.length + "/5]"
